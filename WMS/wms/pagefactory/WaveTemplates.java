...
public class WaveTemplates extends TestBase{

	@FindBy  (xpath="//span[@name='WaveTemplateView.common_grid_refresh_action']")
	public WebElement RefreshIcon_WaveTemplate;

	@FindBy  (xpath="//span[@name='WaveTemplateSearchScreen.common_grid_refresh_action']")
	public WebElement RefreshIcon_WaveTemplateSearches;
	
	@FindBy(xpath = "//div[@name='WaveTemplateView.grid']/div[1]//table//tr/th")
	public List<WebElement> columnHeaderElements;
	
	@FindBy(xpath = "//div[@name='WaveTemplateView.grid']/div[2]//table//tr")
	public List<WebElement> rowElements;
	
	@FindBy(xpath = "//div[@name='WaveTemplateSearchScreen.grid']/div[1]//table//tr/th")
	public List<WebElement> columnHeaderElements_WaveTemSearches;
	
	@FindBy(xpath = "//div[@name='WaveTemplateSearchScreen.grid']/div[2]//table//tr")
	public List<WebElement> rowElements_WaveTemSearches;
	
	@FindBy  (xpath="//span[text()='Wave Template Searches']")
	public WebElement WaveTemplatesSearches_Btn;
	
	@FindBy  (xpath="//span[@name='WaveTemplateSearchScreen.common_grid_edit_action']")
	public WebElement EditBtn_WaveTemplatesSearches;
	
	@FindBy  (xpath="//label[text()='Order Nbr']/ancestor::tr[1]//input")
	public WebElement OrderNum_WaveTemplateSearch;
	
	@FindBy  (xpath="//span[@name='WaveTemplateSearchScreen.detail_pane.save']")
	public WebElement SaveBtn_WaveTemplatesSearches;
	
	@FindBy  (xpath="//span[text()='Wave Templates' and contains(@id, 'Button')]")
	public WebElement WaveTemplates_Btn;
	
	@FindBy  (xpath="//span[@name='WaveTemplateView.run']")
	public WebElement RunWaveTemplate_Btn;
	...
	/*public String getTableCellValue(String strHeaderName) {
		List<WebElement> colHeaders = null;
		int iColNo;
		
		try {
			colHeaders = driver.findElements(By.xpath("//div[@name='WaveTemplateView.grid']/div[1]//table//tr/th"));
			//Get the Column # of the Column_Name
			System.out.println("No of Column Headers: "+colHeaders.size());
			
			for (iColNo = 0; iColNo < colHeaders.size(); iColNo++) {
				String headerName = colHeaders.get(iColNo).getText();
				System.out.println(headerName+"-- Col# --"+iColNo);
				if(headerName.equalsIgnoreCase(strHeaderName)) {
					iColNo = iColNo + 1;
					System.out.println("Column Name: "+headerName+"-- Found in Col# --"+iColNo);
					break;
				}
		...
		int iRowNo = 0;
		try {
			rowValues = driver.findElements(By.xpath("//div[@name='WaveTemplateView.grid']/div[2]//table//tr/td["+iColNo+"]"));
			//Get the Row value of the Column# 'i' 
			System.out.println("No of Rows in DataTable: "+rowValues.size());
			
			if(rowValues.size() > 0) {
				String rowText = rowValues.get(iRowNo).getText();
				System.out.println("Row Value: "+rowText+"-- Row# --"+(iRowNo+1));
				return rowText;
			}else {
				log.info("No of Row is: 0");
	...
	public String getRunNumberFromSuccessDialog() {
		if(cmnLib.waitForElementToBeVisible(SuccessDialog)) {
			String strSuccessMSG = SuccessDialog.getText().trim();
			String strRunNumber = strSuccessMSG.split(" ")[1];
			System.out.println("Run Number: "+strRunNumber);
			return strRunNumber;
		}else {
			log.info("Success Dialog WebElement does not exist");
		}
		return null;
	}
