...
@Test
public class WMS_INV003_5_Unlock_Location extends TestBase {



	String strModule = "WMS";

	public void WMS_INV003_5_Unlock_Location_TC() throws Throwable {

		rpt = new ReportGeneration("WMS_INV003_5_Unlock_Location", "Unlock Location");

		String strDataSheetName = "INV003_5";
		...
		{
			//PASS
			rpt.generateReport("WMS_INV003_5_Unlock_Location", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("WMS_INV003_5_Unlock_Location", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
		...
		//Edit Lock code
			...
			location.LockCode_Edit.clear();
			if(cmnLib.enterDataInTextBox(location.LockCode_Edit, exl.read(strDataSheetName, dataRow, "LockCode"), true))
			{
				/*cmnLib.enterDataInTextBox(location.LockCode_Edit, null, true);
				cmnLib.pressEnterKey();*/
				/*location.LockCode_Edit.clear();
				location.LockCode_Edit.sendKeys(Keys.TAB);*/
				location.LockCode_Edit.sendKeys(Keys.CONTROL + "a");
				location.LockCode_Edit.sendKeys(Keys.DELETE);
				rpt.generateReport("", "Lock Code", "", "", "Lock Code must be removed", "Lock Code is removed", "Passed", "", true );
				TimeUnit.SECONDS.sleep(2);
				cmnLib.clickOnWebElement(location.Save_Button);
			}else {
				rpt.generateReport("", "Lock Code", "", "", "Lock Code must be removed", "Lock Code is NOT removed", "Failed", "", true );
				Assert.fail("Lock Code is NOT removed");
			}
			cmnLib.waitForPageLoaded();
			TimeUnit.SECONDS.sleep(3);
			cmnLib.moveToElementAndClick(location.LockCode_Value);
			TimeUnit.SECONDS.sleep(3);
			if(location.LockCode_Value.getText().equals(""))
			{
				rpt.generateReport("", "Unlock Location", "", "", "Successfully Unlocked Location", "Successfully Unlocked Location", "Passed", "", true );
			}else {
				rpt.generateReport("", "Unlock Location", "", "", "Successfully Unlocked Location", "Failed to Unlock Location", "Failed", "", true );
				Assert.fail("Failed to Unlock Location");
			}
