...
import finance.pagefactory.AR_Reconcilitation;
...
public class FIN_PERU_GL_13_Transfer_And_Conciliation_AR_GL extends TestBase {

	String strModule = "FIN";

	@Test
	public void FIN_PERU_GL_13_Transfer_And_Conciliation_AR_GL_TC() throws Throwable {

		boolean login = false;

		rpt = new ReportGeneration("FIN_PERU_GL_13", "Transfer_And_Conciliation_AR_GL");
		...
		if (!(new HomePage_FIN().getUserNameFromHomePage() != null && new HomePage_FIN().getUserNameFromHomePage()
			...
			if (driver.getTitle().contains("Sign In")) {
				rpt.generateReport("FIN_PERU_GL_13_Transfer_And_Conciliation_AR_GL", "Navigate to Url", "", URL,
		...
		if (!login) {
			...
			rpt.generateReport("FIN_PERU_GL_13_Transfer_And_Conciliation_AR_GL", "Login to Application", "",
		...
		for (int dataRow = 1; dataRow < exl.getRowCount(sheetName); dataRow++) {
			...
			//Validation of Confirmation Message
		     ...
		     if(cmnLib.clickOnWebElement(scheduledProcesses.OK_Confirmation) == false) 
		        ...
		        AR_Reconcilitation aR_Reconcilitation= new AR_Reconcilitation();
		        
		        TimeUnit.SECONDS.sleep(2);
				rpt.enterStepHeader("Navigate to Receivables -> Accounts Receivable");

				if (cmnLib.clickOnWebElement(homepage.NavigatorIcon) && cmnLib.clickOnWebElement(aR_Reconcilitation.AccountReceivable)) {

					Thread.sleep(1000);
					rpt.generateReport("", "Click on Navigator", "", "", "Navigator must be clicked",
							"Clicked on Navigator", "Passed", "", false);
					rpt.generateReport("", "Click on Account Receivable link", "", "", "Account Receivable link must be clicked",
							"Clicked on Account Receivable link", "Passed", "", true);
				} else {
					rpt.generateReport("", "Navigate to Account Receivable page", "Click Navigator --> Account Receivable", "",
							"Application must navigate to Account Receivable page", "Account Receivable link not clicked", "Failed", "", true);
					Assert.fail("Failed to click Account Receivable link");
					
				}
				
				TimeUnit.SECONDS.sleep(5);
				rpt.enterStepHeader("Navigate to Receivables to Ledger Reconciliation");

				if (cmnLib.clickOnWebElement(aR_Reconcilitation.TaskPane)
						&& cmnLib.clickOnWebElement(aR_Reconcilitation.ReceivablesReconciliation)) {

					rpt.generateReport("", "Click on Task Pane", "", "", "Task Pane must be clicked",
							"Clicked on Task Pane", "Passed", "", false);
					rpt.generateReport("", "Click on Receivables to Ledger Reconciliation link", "", "", "Receivables to Ledger Reconciliation link must be clicked",
							"Clicked on Receivables to Ledger Reconciliation link", "Passed", "", true);
				} else {
					rpt.generateReport("", "Click on Receivables to Ledger Reconciliation in Task Pane", "", "",
							"Receivables to Ledger Reconciliation link must be clicked", "Not clicked on Receivables to Ledger Reconciliation link", "Failed", "",
							true);
					Assert.fail("Failed to click Receivables to Ledger Reconciliation link");
				}
				...
				if(cmnLib.waitForElementToBeVisible(aR_Reconcilitation.ReceivablesHeader) == true)
				{
					rpt.generateReport("", "Navigation to Receivables to Ledger Reconciliation page", "", "", "Receivables to Ledger Reconciliation page should be displayed", "Receivables to Ledger Reconciliation page Is displayed", "Passed", "", true );
				...
				{
					//FAIL
					rpt.generateReport("", "Navigation to Receivables to Ledger Reconciliation page", "", "", "Receivables to Ledger Reconciliation page should not be displayed", "Receivables to Ledger Reconciliation page Is Not displayed", "Failed", "", true );
					Assert.fail("Receivables to Ledger Reconciliation page is not displayed");
				}	
