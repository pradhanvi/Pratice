...
/**
Script Name			: FIN_PERU_CL_06_ReconcileBankStatementsTransactionsAutomatic
Script Description	: Reconcile Bank Statements to Transactions (Automatic)
Track/Module		: FIN
Sub Track			: 
...
public class FIN_PERU_CL_06_ReconcileBankStatementsTransactionsAutomatic extends TestBase{
	String strModule = "FIN";

	@Test
	public void FIN_PERU_CL_06_ReconcileBankStatementsTransactionsAutomatic_TC() throws Throwable { 
		rpt = new ReportGeneration("FIN_PERU_CL_06_ReconcileBankStatementsTransactionsAutomatic", "");

		String strDataSheetName = "CL_06";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_FIN.HomePage_HomeIcon))
		{
			...
			rpt.generateReport("FIN_PERU_CL_06_ReconcileBankStatementsTransactionsAutomatic", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", false );
		...
		{
			//FAIL
			rpt.generateReport("FIN_PERU_CL_06_ReconcileBankStatementsTransactionsAutomatic", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
		...
		for (int dataRow = 1; dataRow < exl.getRowCount(strDataSheetName); dataRow++) {
			...
			if(cmnLib.clickOnWebElement(bankstmtreconc.SubmitAutoreconciliation)) {
				rpt.generateReport("", "Click on Submit Autoreconciliation link in Tasks", "", "", "Submit Autoreconciliation Link should be clicked", "Submit Autoreconciliation link is clicked", "Passed", "", true);
			}else {
				rpt.generateReport("", "Click on Submit Autoreconciliation link in Tasks", "", "", "Submit Autoreconciliation Link should be clicked", "Submit Autoreconciliation link is not clicked", "Failed", "", true);
				Assert.fail("Submit Autoreconciliation link is not clicked");
			}
			cmnLib.waitForPageLoaded();
			
			if(cmnLib.waitForElementToBeVisible(bankstmtreconc.BankAccount)) {
				rpt.generateReport("", "Verify Autoreconcile Bank Statements page", "", "", "Autoreconcile Bank Statements page should be opened", "Autoreconcile Bank Statements page is opened", "Passed", "", true);
			}else {
				rpt.generateReport("", "Verify Autoreconcile Bank Statements page", "", "", "Autoreconcile Bank Statements page should be opened", "Autoreconcile Bank Statements page is not opened", "Failed", "", true);
				Assert.fail("Autoreconcile Bank Statements page is not opened");
			}
			...
			cmnLib.enterDataInTextBox(bankstmtreconc.FromStatementEndDate, cmnLib.futureDate("dd/MMM/yyyy", 0), true);
			cmnLib.enterDataInTextBox(bankstmtreconc.ToStatementEndDate, cmnLib.futureDate("dd/MMM/yyyy", 0), true);
			
			
			rpt.generateReport("", "Enter Details in Autoreconcile Bank Statements page", "", "", "All Mandatory Details should be entered", "All Mandatory details are entered", "Passed", "", true);
			
			if(cmnLib.clickOnWebElement(bankstmtreconc.SubmitBtn))
			{
				rpt.generateReport("", "Submit the process", "", "", "Click on Submit Button", "Clicked on Submit Button.Successfully Submitted", "Passed", "", true);
			}else {
				rpt.generateReport("", "Submit the process", "", "", "Click on Submit Button", "Not Clicked on Submit Button.Failed to Submit the process", "Passed", "", true);
				Assert.fail("Not Clicked on Submit Button.Failed to Submit the process");
			}
			TimeUnit.SECONDS.sleep(3);
			if(cmnLib.waitForElementToBeVisible(bankstmtreconc.ConfirmationDialogBox,20)) {
				rpt.generateReport("", "Confirmation message", "", "", "Confirmation message must appear", "Confirmation message appeared", "Passed", "", true);
				cmnLib.clickOnWebElement(bankstmtreconc.ConfirmSubmitDialog_Ok);
			}else {
				rpt.generateReport("", "Confirmation message", "", "", "Confirmation message must appear", "Confirmation message did not appear", "Failed", "", true);
