...
import otm.pagefactory.Buy_Shipment;
...
/**
Script Name			: OTM_CHILE_LAS073_Select_Add_Shipment
Script Description	: How to Select and Add Shipement
Track/Module		: OTM
Sub Track			: 
...
public class OTM_CHILE_LAS073_Select_Add_Shipment extends TestBase {
	...
	public void f() throws Throwable {
		rpt = new ReportGeneration("OTM_CHILE_LAS073_Select_Add_Shipment", "How to Select and Add Shipement");
		String sheetName = "LAS073";
		...
		if (homepage != null) {
			rpt.generateReport("OTM_CHILE_LAS073_Select_Add_Shipment", "Enter Username", "",
			...
			rpt.generateReport("OTM_CHILE_LAS073_Select_Add_Shipment", "Login to application",
		...
		for (int dataRow = 1; dataRow < exl.getRowCount(sheetName); dataRow++) {
			...
			//Select Query drop down
				...
				//Clicking on Add Icon
					    ...
					    if(cmnLib.clickOnWebElement(dispatchboard.AddButton))
					    {
							   rpt.generateReport("", "Click on Add Icon", "", "", "Add Icon must be clicked","Add Icon is clicked", "Passed", "", true);
						 }else {
							   rpt.generateReport("", "Click on Add Icon", "", "", "Add Icon must be clicked","Add Icon is NOT clicked", "Failed", "", true);
							   Assert.fail("Add Icon is NOT clicked");
						 }
					    
					    cmnLib.switchToFrame("dcc:j_id7:selAddIF", FrameSelectBy.StringFrameName);
					    
					    cmnLib.enterDataInTextBox(dispatchboard.ShipmentID_Add, exl.read(sheetName, dataRow, "Shipment_ID_Replace"), true);
					    cmnLib.clickOnWebElement(dispatchboard.SearchButton_Add);
					    
					//Selecting the shipments to replace
					    
					    Buy_Shipment buyShipments=new Buy_Shipment();
					    
					   if( buyShipments.selectOrderFromResults(exl.read(sheetName, dataRow, "Shipment_ID_Replace")))
					   {
						   rpt.generateReport("", "Shipment selection", "", exl.read(sheetName, dataRow, "Shipment_ID_Replace"), "Shipments should be selected","Shipments are selected", "Passed", "", true);
					   }else {
						   rpt.generateReport("", "Shipment selection", "", exl.read(sheetName, dataRow, "Shipment_ID_Replace"), "Shipments should be selected","Shipments are NOT selected", "Failed", "", true);
						   Assert.fail("Shipments are NOT selected");
					   }
					   
					   cmnLib.clickOnWebElement(dispatchboard.ReplaceButton);
					   
				//Verifying the replaced shipment
					   ...
					   if(dispatchboard.verifySearchedRecordExists(exl.read(sheetName, dataRow, "Shipment_ID_Replace")))
					   {
						   rpt.generateReport("", "Verify Replaced Shipments", "", exl.read(sheetName, dataRow, "Shipment_ID_Replace"), "Replaced Shipments should be selected","Replaced Shipments are selected", "Passed", "", true);
					   }else {
						   rpt.generateReport("", "Shipment selection", "", exl.read(sheetName, dataRow, "Shipment_ID_Replace"), "Replaced Shipments should be selected","Replaced Shipments are NOT selected", "Failed", "", true);
						   Assert.fail("Replaced Shipments are NOT selected");
					   }   
