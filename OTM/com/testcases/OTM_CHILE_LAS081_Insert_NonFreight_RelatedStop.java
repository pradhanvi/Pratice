...
/**
Script Name			: OTM_CHILE_LAS081_Insert_NonFreight_RelatedStop
Script Description	: How to Insert Non Freight Related Stop
Track/Module		: OTM
Sub Track			: 
...
public class OTM_CHILE_LAS081_Insert_NonFreight_RelatedStop extends TestBase {


	String strModule = "OTM_CHILE";

	@Test 
	public void OTM_CHILE_LAS081_Insert_NonFreight_RelatedStop_TC() throws Throwable {
		rpt = new ReportGeneration("OTM_CHILE_LAS081_Insert_NonFreight_RelatedStop", "How to Insert Non Freight Related Stop");
		String sheetName = "LAS081";
		...
		if (homepage != null) {
			rpt.generateReport("OTM_CHILE_LAS081_Insert_NonFreight_RelatedStop", "Enter Username", "",
			...
			rpt.generateReport("OTM_CHILE_LAS081_Insert_NonFreight_RelatedStop", "Login to application",
		...
		for (int dataRow = 1; dataRow < exl.getRowCount(sheetName); dataRow++) {
			...
			//Insert Non Freight Related Stop
					    ...
					    if(cmnLib.waitForElementToBeVisible(dispatchboard.NFRLocationID))
						   {
							   rpt.generateReport("", "Insert Non Freight Related Stop", "", "", "Insert Non Freight Related Stop window must appear","Insert Non Freight Related Stop window appeared", "Passed", "", true);
						   }else if(cmnLib.waitForElementToBeVisible(dispatchboard.ErrorWindow))
						   {
							   rpt.generateReport("", "Insert Non Freight Related Stop", "", "", "Insert Non Freight Related Stop window must appear","Insert Non Freight Related Stop window did NOT appear", "Failed", "", true);
							   Assert.fail("Insert Non Freight Related Stop window did NOT appear");
						   } 
					    
					    cmnLib.enterDataInTextBox(dispatchboard.NFRLocationID, exl.read(sheetName, dataRow, "NFRLocationID"), true);
