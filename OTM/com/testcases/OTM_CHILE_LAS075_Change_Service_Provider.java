...
/**
Script Name			: OTM_CHILE_LAS075_Change_Service_Provider
Script Description	: How to Change Service provide for shipments
Track/Module		: OTM
Sub Track			: 
...
public class OTM_CHILE_LAS075_Change_Service_Provider extends TestBase {
	String strModule = "OTM_CHILE";

	@Test 
	public void OTM_CHILE_LAS075_Change_Service_Provider_TC() throws Throwable {
		rpt = new ReportGeneration("OTM_CHILE_LAS075_Change_Service_Provider", "How to Change Service provide for shipments");
		String sheetName = "LAS075";
		...
		if (homepage != null) {
			rpt.generateReport("OTM_CHILE_LAS075_Change_Service_Provider", "Enter Username", "",
			...
			rpt.generateReport("OTM_CHILE_LAS075_Change_Service_Provider", "Login to application",
			...
			//Select Query drop down
				...
				//Clicking on Actions Icon
					 ...
					 //Switching frame to Actions  
					    ...
					    cmnLib.clickOnWebElement(dispatchboard.ChangeServiceProvider);
					    cmnLib.waitForPageLoaded();
					    
					    cmnLib.switchToWindowTab("Service Providers");
					    /*List<WebElement> list = driver.findElements(By.tagName("frame"));
					    System.out.println("# of Frames: "+list.size());
					   for (int i = 0; i < list.size(); i++) {
						System.out.println("Frame - "+list.get(i).getAttribute("name"));
						System.out.println("Frame - "+list.get(i).getAttribute("id"));
					}*/
					   ...
					   if(dispatchboard.verifyServiceProviderListExsits())
					   {
						   rpt.generateReport("", "Service Providers", "", "", "Service provider available list must be displayed","Service provider available list are displayed", "Passed", "", true);
					   }else {
						   rpt.generateReport("", "Service Providers", "", "", "Service provider available list must be displayed","Service provider available list are NOT displayed", "Failed", "", true);
						   Assert.fail("Service provider available list are NOT displayed");
					   } 
