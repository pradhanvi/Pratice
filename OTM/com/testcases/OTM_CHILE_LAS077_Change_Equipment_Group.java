...
/**
Script Name			: OTM_CHILE_LAS077_Change_Equipment_Group
Script Description	: Change Equipment Group
Track/Module		: OTM
Sub Track			: 
...
public class OTM_CHILE_LAS077_Change_Equipment_Group extends TestBase {

	String strModule = "OTM_CHILE";

	@Test 
	public void OTM_CHILE_LAS077_Change_Equipment_Group_TC() throws Throwable {
		rpt = new ReportGeneration("OTM_CHILE_LAS077_Change_Equipment_Group", "Change Equipment Group");
		String sheetName = "LAS077";
		...
		if (homepage != null) {
			rpt.generateReport("OTM_CHILE_LAS077_Change_Equipment_Group", "Enter Username", "",
			...
			rpt.generateReport("OTM_CHILE_LAS077_Change_Equipment_Group", "Login to application",
		...
		for (int dataRow = 1; dataRow < exl.getRowCount(sheetName); dataRow++) {
			...
			//Select Query drop down
			 ...
			 //Switching frame to Actions  
					    ...
					    cmnLib.clickOnWebElement(dispatchboard.ChangeEquipmentGroup);
					    cmnLib.waitForPageLoaded();
					    
					    cmnLib.switchToWindowTab("Change Equipment Group");
					    if(cmnLib.switchToFrame("mainBody", FrameSelectBy.StringFrameName)) {
					    	System.out.println("Switch to Frame: mainBody - Successfull...");
					    }
				//Selecting the Equipment Group
					    
					    if(cmnLib.enterDataInTextBox(dispatchboard.EquipmentGroupID_Input, exl.read(sheetName, dataRow, "EquipmentGroupID_Input"), true))
					    {
							   rpt.generateReport("", "Equipment Group", "", exl.read(sheetName, dataRow, "EquipmentGroupID_Input"), "Equipment Group must be selected","Equipment Group is selected", "Passed", "", true);
						   }else if(cmnLib.waitForElementToBeVisible(dispatchboard.ErrorWindow))
						   {
							   rpt.generateReport("", "Equipment Group", "", exl.read(sheetName, dataRow, "EquipmentGroupID_Input"), "Equipment Group must be selected","Equipment Group is NOT selected", "Failed", "", true);
							   Assert.fail("Equipment Group is NOT selected");
						   }
					   ...
					   {
						   rpt.generateReport("", "Equipment Group", "", exl.read(sheetName, dataRow, "EquipmentGroupID_Input"), "Affected window must appear. Equipment Group has changed successfully","Affected Window appeared. Equipment Group has changed successfully", "Passed", "", true);
					   }else if(cmnLib.waitForElementToBeVisible(dispatchboard.ErrorWindow))
					   {
						   rpt.generateReport("", "Equipment Group", "", exl.read(sheetName, dataRow, "EquipmentGroupID_Input"), "Error window must appear. Equipment Group has NOT changed","Error window appeared. Equipment Group has NOT changed", "Failed", "", true);
						   Assert.fail("Error window appeared. Equipment Group has NOT changed");
					   }
