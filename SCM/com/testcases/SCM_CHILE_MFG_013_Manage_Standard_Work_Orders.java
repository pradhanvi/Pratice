...
/**
Script Name			: SCM_CHILE_MFG_013_Manage_Standard_Work_Orders
Script Description	: Manage Standard Work Orders
Track/Module		: SCM
Sub Track			: 
...
public class SCM_CHILE_MFG_013_Manage_Standard_Work_Orders extends TestBase{
	String strModule = "SCM";
	@Test
	public void SCM_CHILE_MFG_013_Manage_Standard_Work_Orders() throws Throwable {
		rpt = new ReportGeneration("SCM_CHILE_MFG_013_Manage_Standard_Work_Orders", "Manage Standard Work Orders");

		String strDataSheetName = "MFG_013";
		boolean login = false;
		...
		if(cmnLib.waitForElementToBeVisible(homePage_SCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("SCM_CHILE_MFG_013_Manage_Standard_Work_Orders", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", false );
		...
		{
			//FAIL
			rpt.generateReport("SCM_CHILE_MFG_013_Manage_Standard_Work_Orders", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Adding New Standard Work Order
			rpt.enterStepHeader("Adding New Standard Work Order");
			cmnLib.clickOnWebElement(manageWorkOrders.AddNewWorderOrderDropdown);
			cmnLib.clickOnWebElement(manageWorkOrders.CreateStandardWorkOrderLink);
			TimeUnit.SECONDS.sleep(2);
			if(cmnLib.waitForElementToBeVisible(manageWorkOrders.Item_WorkOrder))
			{
				cmnLib.enterDataInTextBox(manageWorkOrders.Item_WorkOrder, exl.read(strDataSheetName, dataRow, "Item"), true);
				TimeUnit.SECONDS.sleep(1);
				cmnLib.enterDataInTextBox(manageWorkOrders.Quanity_WorkOrder, exl.read(strDataSheetName, dataRow, "Quantity"), true);
				rpt.generateReport("", "Create Standard Work Order", "", "", "Standard work order details must be entered", "Standard work order details are entered", "Passed", "", true );
				cmnLib.clickOnWebElement(manageWorkOrders.SaveEditButton_WorkOrder);
			}else {
				rpt.generateReport("", "Create Standard Work Order", "", "", "Create Standard Work Order window must appear", "Create Standard Work Order window did not appear", "Failed", "", true );
				Assert.fail("Create Standard Work Order window did not appear");
			}
			...
			if(cmnLib.waitForElementToBeVisible(manageWorkOrders.WorkOrderGenralInfoTab))
			{
				...
				rpt.generateReport("", "Create Standard Work Order", "", "", "General information tab must appear", "General information tab appeared", "Passed", "", true );
				cmnLib.clickOnWebElement(manageWorkOrders.WorkOrderOperationsTab);
				TimeUnit.SECONDS.sleep(2);
				rpt.generateReport("", "Create Standard Work Order", "", "", "Opeartions tab must appear", "Opeartions tab appeared", "Passed", "", true );
				
				cmnLib.clickOnWebElement(manageWorkOrders.SaveCloseButton_WorkOrder);
			}else {
				rpt.generateReport("", "Create Standard Work Order", "", "", "Work Order details must appear", "Work Order details did not appear", "Failed", "", true );
		...
		//Verifying Confirmation message
			...
			if(manageWorkOrders.ConfirmationTD.isDisplayed())
			{
				...
				{
					rpt.generateReport("", "Standard Work Order", "", "", "Standard work order must be created successfully", "Standard work order created successfully", "Passed", "", true );
				}else {
					rpt.generateReport("", "Standard Work Order", "", "", "Standard work order must be created successfully", "Standard work order was not created successfully", "Failed", "", true );
					Assert.fail("Standard work order was not created successfully");
				}
			}else {
				rpt.generateReport("", "Standard Work Order", "", "", "Standard work order must be created successfully", "Standard work order was not created successfully", "Failed", "", true );
				Assert.fail("Standard work order was not created successfully");
