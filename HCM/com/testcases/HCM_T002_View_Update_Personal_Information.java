...
public class HCM_T002_View_Update_Personal_Information extends TestBase{
	String strModule = "HCM";
	
	@Test
	public void HCM_T002_View_Update_Personal_Information_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_T002_View_Update_Personal_Information", "View and Update Personal Information");

		String strDataSheetName = "T002_Vi_Upd_PI";
		boolean login = false;
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_T002_View_Update_Personal_Information", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_T002_View_Update_Personal_Information", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
		...
		for (int dataRow = 1; dataRow < exl.getRowCount(strDataSheetName); dataRow++) {
			...
			//Selecting Options from Edit dropdown
				...
				rpt.generateReport("", "Click on Update Name OK Button", "", "", "Must Click on OK button", "Clicked on OK button", "Pass", "", true);
				managePerson.clickUpdateNameOkButton();
				TimeUnit.SECONDS.sleep(3);
