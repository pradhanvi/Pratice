...
/**
Script Name			: HCM_CHILE_T005_4_AddPersonalRelationship
Script Description	: Add Relationship - Delete Personal Relationship from HR Specialist
Track/Module		: HCM
Sub Track			: 
...
public class HCM_CHILE_T005_4_AddPersonalRelationship_DeleteFrmHRSplt extends TestBase {


	String strModule = "HCM";
	
	@Test
	public void HCM_CHILE_T005_4_AddPersonalRelationship_DeleteFrmHRSplt_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T005_4_AddPersonalRelationship", "Delete Personal Relationship from HR Specialist");

		String strDataSheetName = "T005_4_AddPersoRelatshp";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T005_4_AddPersonalRelationship_DeleteFrmHRSplt", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T005_4_AddPersonalRelationship_DeleteFrmHRSplt", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Clicking on Contact Tab
			...
			//Selecting the Contact to Delete
				if(manageContact.ClickOnSearchedResults(exl.read(strDataSheetName, dataRow, "PaternalLastName_Delete")) == true)
				{
					rpt.generateReport("", "Click on Person Name", "", exl.read(strDataSheetName, dataRow, "PaternalLastName_Delete"), "Person Name of the Searched Employee should be clicked", "Person Name of the Searched Employee is clicked", "Passed", "", true);
				}else
				{
					rpt.generateReport("", "Click on Person Name", "", exl.read(strDataSheetName, dataRow, "PaternalLastName_Delete"), "Person Name of the Searched Employee should be clicked", "Person Name of the Searched Employee is not clicked", "Failed", "", true);
			...
			//Delete Contacts
				...
				cmnLib.waitForElementToBeVisible(manageContact.DeleteContact, 20);
				cmnLib.clickOnWebElement(manageContact.DeleteContact);
				
				if(cmnLib.waitForElementToBeVisible(manageContact.DeleteConfirmationPopUp, 20))
				{
					rpt.generateReport("", "Delete Personal relationship contact details", "", exl.read(strDataSheetName, dataRow, "PaternalLastName_Delete"), "Delete Confirmation Pop up should appear", "Delete Confirmation Pop up appeared", "Passed", "", true);
					TimeUnit.SECONDS.sleep(2);
					cmnLib.clickOnWebElement(manageContact.DeleteConfirmationPopUp_Yes);
				}
			...
			//Submit 
				...
				{
					rpt.generateReport("", "Delete Personal relationship contact", "", "", "Warning message must Pop up", "Warning message is Popping up", "Passed", "", true);
		...
		//Confirmation
				...
				{
					rpt.generateReport("", "Delete Personal relationship contact", "", "", "Confirmation message must Pop up", "Confirmation message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(manageContact.Ok_Confirmation);
				}else {
					rpt.generateReport("", "Delete Personal relationship contact", "", "", "Confirmation message must Pop up", "Confirmation message has not Popped up", "Failed", "", true);
