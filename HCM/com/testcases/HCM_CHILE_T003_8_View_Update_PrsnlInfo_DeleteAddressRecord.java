...
/**
Script Name			: HCM_CHILE_T003_8_View_Update_PrsnlInfo_DeleteAddressRecord
Script Description	: View and Update Personal Information - Delete Address Record
Track/Module		: HCM
Sub Track			: 
...
public class HCM_CHILE_T003_8_View_Update_PrsnlInfo_DeleteAddressRecord extends TestBase {


	String strModule = "HCM";
	
	@Test
	public void HCM_CHILE_T003_8_View_Update_PrsnlInfo_DeleteAddressRecord_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T003_8_View_Update_PrsnlInfo", "Delete Address Record");

		String strDataSheetName = "T003_8_ViewUpdate_PrsnInfo";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T003_8_View_Update_PrsnlInfo_DeleteAddressRecord", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T003_8_View_Update_PrsnlInfo_DeleteAddressRecord", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Delete address record
				...
				managePerson.clickAddressEditSelectDeleteRecord();
				...
				//Commented after 12B upgrade
				/*if(cmnLib.waitForElementToBeVisible(managePerson.DeleteRecordConfirmationYes, 10))
				{
					rpt.generateReport("", "Delete Confirmation message", "", "", "Must Click on OK button", "Clicked on OK button", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.DeleteRecordConfirmationYes);
				}else if(cmnLib.waitForElementToBeVisible(managePerson.DeleteConfirmationRadio, 10))
				{
					rpt.generateReport("", "Selecting primary address", "", "", "Should Select address ", "Selecting the primary address", "Passed", "", true);
					cmnLib.clickOnWebElement(managePerson.DeleteRecordConfirmationYes);
				}else {
					rpt.generateReport("", "Delete Confirmation message", "", "", "Must Click on OK button", "Error occured while deleting address", "Failed", "", true);
					Assert.fail("Error occured while deleting address");
				}*/
		...
		//Submit 
				...
				{
					rpt.generateReport("", "Delete address record", "", "", "Warning message must Pop up", "Warning message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
		...
		//Confirmation
				...
				{
					rpt.generateReport("", "Delete address record", "", "", "Confirmation message must Pop up", "Confirmation message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.Ok_Confirmation);
				}else {
					rpt.generateReport("", "Delete address record", "", "", "Confirmation message must Pop up", "Confirmation message has not Popped up", "Failed", "", true);
