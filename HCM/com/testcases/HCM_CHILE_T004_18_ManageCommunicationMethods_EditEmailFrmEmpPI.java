...
/**
Script Name			: HCM_CHILE_T004_18_ManageCommunicationMethods_EditEmailFrmEmpPI
Script Description	: Manage Communication methods - Edit Email from Emp PI
Track/Module		: HCM
Sub Track			: 
...
public class HCM_CHILE_T004_18_ManageCommunicationMethods_EditEmailFrmEmpPI extends TestBase {
	String strModule = "HCM_EMP";
	
	@Test
	public void HCM_CHILE_T004_18_ManageCommunicationMethods_EditEmailFrmEmpPI_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T004_18_ManageCommunicationMethods", "Edit Email from Emp PI");

		String strDataSheetName = "T004_18_ManageCommMthds";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T004_18_ManageCommunicationMethods_EditEmailFrmEmpPI", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T004_18_ManageCommunicationMethods_EditEmailFrmEmpPI", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Click on Edit Button
			...
			//cmnLib.clickOnWebElement(personalInfo.SelectEmail);
			TimeUnit.SECONDS.sleep(3);
			
			personalInfo.selectOptionFromComboBoxInsideTable(personalInfo.EmailType, exl.read(strDataSheetName, dataRow, "EmailType"));
			cmnLib.enterDataInTextBox(personalInfo.EmailInput, exl.read(strDataSheetName, dataRow, "EmailID"), true);
			
			rpt.generateReport("", "Add Email ID", "", "", "All the details must be entered ", "All the details are entered", "Passed", "", true);
			TimeUnit.SECONDS.sleep(3);
			...
			{
				rpt.generateReport("", "Edit Email ID", "", "", "Email ID should be Edited", "Successfully Edited Email ID", "Passed", "", true);
			}else {
				rpt.generateReport("", "Edit Email ID", "", "", "Email ID should be Edited", "Error occured while Edited Email ID", "Failed", "", true);
				Assert.fail("Error occured while Edited Email ID");
			}
