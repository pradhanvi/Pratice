...
/**
Script Name			: HCM_CHILE_T003_16_View_Update_PrsnlInfo_CorrectGenderAndMartialStatus
Script Description	: View and Update Personal Information - Correct Gender and Martial status
Track/Module		: HCM
Sub Track			: 
...
public class HCM_CHILE_T003_16_View_Update_PrsnlInfo_CorrectGenderAndMartialStatus extends TestBase {


	String strModule = "HCM";
	
	@Test
	public void HCM_CHILE_T003_16_View_Update_PrsnlInfo_CorrectGenderAndMartialStatus_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T003_16_View_Update_PrsnlInfo", "Correct Gender and Martial status");

		String strDataSheetName = "T003_16_ViewUpdate_PrsnInfo";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T003_16_View_Update_PrsnlInfo_CorrectGenderAndMartialStatus", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T003_16_View_Update_PrsnlInfo_CorrectGenderAndMartialStatus", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Correct Gender and Martial status
				
				managePerson.clickGenderMartialStatusEditSelectCorrect();
				TimeUnit.SECONDS.sleep(3);
				/*managePerson.EffectiveStartDate.clear();
				managePerson.EffectiveStartDate.sendKeys(managePerson.futureDate("dd/MMM/YYYY",0));
				rpt.generateReport("", "Click on Update Name OK Button", "", "", "Must Click on OK button", "Clicked on OK button", "Passed", "", true);
				cmnLib.clickOnWebElement(managePerson.UpdateGenderEffectiveDateOKButton);
				TimeUnit.SECONDS.sleep(3);*/
				
				managePerson.selectValueFromInputComboBox("Gender", exl.read(strDataSheetName, dataRow, "Gender"));
				managePerson.selectValueFromInputComboBox("Marital Status", exl.read(strDataSheetName, dataRow, "MaritalStatus"));
				managePerson.selectValueFromInputComboBox("Highest Education Level", exl.read(strDataSheetName, dataRow, "HighestEducationLevel"));
				rpt.generateReport("", "Correct Gender and Martial status", "", "", "Should Enter all the required details", "All the required details are entered", "Passed", "", true);
				TimeUnit.SECONDS.sleep(3);
				cmnLib.clickOnWebElement(managePerson.OKUpdateGender);
		...
		//Submit 
				...
				{
					rpt.generateReport("", "Correct Gender and Martial status", "", "", "Warning message must Pop up", "Warning message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
		...
		//Confirmation
				...
				{
					rpt.generateReport("", "Correct Gender and Martial status", "", "", "Confirmation message must Pop up", "Confirmation message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.Ok_Confirmation);
				}else {
					rpt.generateReport("", "Correct Gender and Martial status", "", "", "Confirmation message must Pop up", "Confirmation message has not Popped up", "Failed", "", true);
