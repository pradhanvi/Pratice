...
/**
Script Name			: HCM_CHILE_T005_Manage_Address
Script Description	: Manage Address
Track/Module		: HCM
Sub Track			: 
...
Responsibility		: 
Pre-Requisites		: An Address must be added to employee
Comments (if any)	:
 **/

public class HCM_CHILE_T005_Manage_Address extends TestBase{

	String strModule = "HCM_EMP";
	
	@Test
	public void HCM_CHILE_T005_Manage_Address_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T005_Manage_Address", "Manage Address");

		String strDataSheetName = "T005_Manage_Address";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T005_Manage_Address", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T005_Manage_Address", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Click on Contacts Tab
			...
			cmnLib.clickOnWebElement(personalInformation.ContactsInformation);
			
			if(cmnLib.waitForElementToBeVisible(personalInformation.AddressEdit))
			{
				cmnLib.clickOnWebElement(personalInformation.AddressEdit);
				...
				{
					rpt.generateReport("", "Edit Address", "", "", "Successfully Edited Address ","Successfully Edited Address, ", "Passed", "", true);
				}else {
					
					rpt.generateReport("", "Edit Address", "", "", "Successfully Edited Address, ", "Failed to Edit address", "Failed", "", true);
					Assert.fail("Failed to Edit address");
				}
			}else {
				rpt.generateReport("", "Edit My Details page", "", "", "Edit page must be opened", "Failed to open Edit page", "Failed", "", true);
				Assert.fail("Failed to open Edit page");
			}
