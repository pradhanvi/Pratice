...
/**
Script Name			: HCM_CHILE_T003_13_View_Update_PrsnlInfo_EditBiographicalInfo
Script Description	: View and Update Personal Information - Edits Biographical Info
Track/Module		: HCM
Sub Track			: 
...
public class HCM_CHILE_T003_13_View_Update_PrsnlInfo_EditBiographicalInfo extends TestBase{

	String strModule = "HCM";
	
	@Test
	public void HCM_CHILE_T003_13_View_Update_PrsnlInfo_EditBiographicalInfo_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T003_13_View_Update_PrsnlInfo", "Edits Biographical Info");

		String strDataSheetName = "T003_13_ViewUpdate_PrsnInfo";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T003_13_View_Update_PrsnlInfo_EditBiographicalInfo", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T003_13_View_Update_PrsnlInfo_EditBiographicalInfo", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Update Biographical Info
					...
					cmnLib.clickOnWebElement(managePerson.BiographicalInfoEdit);
					cmnLib.waitForElementToBeVisible(managePerson.DateOfBirth, 10);
					cmnLib.enterDataInTextBox(managePerson.DateOfBirth, exl.read(strDataSheetName, dataRow, "DateOfBirth"), true);
					cmnLib.enterDataInTextBox(managePerson.DateOfDeath, exl.read(strDataSheetName, dataRow, "DateOfDeath"), true);
					cmnLib.enterDataInTextBox(managePerson.CountryOfBirth, exl.read(strDataSheetName, dataRow, "CountryOfBirth"), true);
					cmnLib.enterDataInTextBox(managePerson.RegionOfBirth, exl.read(strDataSheetName, dataRow, "RegionOfBirth"), true);
					cmnLib.enterDataInTextBox(managePerson.TownOfBirth, exl.read(strDataSheetName, dataRow, "TownOfBirth"), true);
					managePerson.selectValueFromInputComboBox("Correspondence Language", exl.read(strDataSheetName, dataRow, "CorrespondenceLanguage"));
					managePerson.selectValueFromInputComboBox("Blood Type", exl.read(strDataSheetName, dataRow, "BloodType"));
					rpt.generateReport("", "Update Biographical Info", "", "", "Should Enter all the required details", "All the required details are entered", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.BiographicalInfoOK);
					
					
		//Submit 
				cmnLib.waitForElementToBeVisible(managePerson.SubmitButton, 30);
				cmnLib.clickOnWebElement(managePerson.SubmitButton);
				
				if(cmnLib.waitForElementToBeVisible(managePerson.Warning_Popup, 30))
				{
					rpt.generateReport("", "Update Biographical Info", "", "", "Warning message must Pop up", "Warning message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.Yes_Warning);
				}
				
		//Confirmation
				if(cmnLib.waitForElementToBeVisible(managePerson.Confirmation_PopUp, 30))
				{
					rpt.generateReport("", "Update Biographical Info", "", "", "Confirmation message must Pop up", "Confirmation message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.Ok_Confirmation);
				}else {
					rpt.generateReport("", "Update Biographical Info", "", "", "Confirmation message must Pop up", "Confirmation message has not Popped up", "Failed", "", true);
