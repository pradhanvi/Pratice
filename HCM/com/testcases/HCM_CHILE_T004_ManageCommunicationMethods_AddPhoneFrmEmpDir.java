...
/**
Script Name			: HCM_CHILE_T004_ManageCommunicationMethods_AddPhoneFrmEmpDir
Script Description	: Manage Communication Methods - Add Phone from Emp Dir
Track/Module		: HCM
Sub Track			: 
...
public class HCM_CHILE_T004_ManageCommunicationMethods_AddPhoneFrmEmpDir extends TestBase {
	String strModule = "HCM_EMP";
	
	@Test
	public void HCM_CHILE_T004_ManageCommunicationMethods_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T004_ManageCommunicationMethods", "Add Phone from Emp Dir");

		String strDataSheetName = "T004_ManageCommMthds";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T004_ManageCommunicationMethods_AddPhoneFrmEmpDir", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T004_ManageCommunicationMethods_AddPhoneFrmEmpDir", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Add Phone number
			...
			driver.findElement(By.xpath("//ul[contains(@id,'1:soc1::pop')]//li[text()='"+exl.read(strDataSheetName, dataRow, "PhoneType")+"']")).click();
			...
			rpt.generateReport("", "Add Phone number", "", "", "All the details must be entered ", "All the details are entered", "Passed", "", true);
			TimeUnit.SECONDS.sleep(3);
			cmnLib.clickOnWebElement(managecomm.OKButton_AddPhone);
			if(!cmnLib.waitForElementToBeVisible(managecomm.ErrorPopUp, 15))
			{
				rpt.generateReport("", "Add Phone number", "", "", "Successfuly add phone number", "Successfuly added phone number", "Passed", "", true);
				TimeUnit.SECONDS.sleep(2);
			}else {
				rpt.generateReport("", "Add Phone number", "", "", "Successfuly add phone number", "Error occured during adding a phone number", "Failed", "", true);
				TimeUnit.SECONDS.sleep(2);
