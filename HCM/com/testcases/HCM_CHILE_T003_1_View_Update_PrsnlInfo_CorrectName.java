...
/**
Script Name			: HCM_CHILE_T003_1_View_Update_PrsnlInfo_CorrectName
Script Description	: View and Update Personal Information - Correct Name
Track/Module		: HCM
Sub Track			: 
...
public class HCM_CHILE_T003_1_View_Update_PrsnlInfo_CorrectName extends TestBase {
	...
	@Test
	public void HCM_CHILE_T003_1_View_Update_PrsnlInfo_CorrectName_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T003_1_View_Update_PrsnlInfo", "Correct Name");

		String strDataSheetName = "T003_1_ViewUpdate_PrsnInfo";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T003_1_View_Update_PrsnlInfo_CorrectName", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T003_1_View_Update_PrsnlInfo_CorrectName", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Selecting Options from Edit dropdown
				...
				managePerson.clickNameEditSelectCorrect();
		...
		//Update Name
				cmnLib.enterDataInTextBox(managePerson.PaternalLastName, exl.read(strDataSheetName, dataRow, "GlobalParentalLastName"), true);
				cmnLib.enterDataInTextBox(managePerson.FirstName, exl.read(strDataSheetName, dataRow, "FirstName"), true);
				cmnLib.clickOnWebElement(managePerson.LocalNameCheckBox);
				TimeUnit.SECONDS.sleep(3);
				cmnLib.clickOnWebElement(managePerson.LocalNameLangDrop);
				SetupAndMaintenance setupandMaintenance=new SetupAndMaintenance();
				
				setupandMaintenance.SelectInputComboBox(exl.read(strDataSheetName, dataRow, "Local_Name_Lang"));
				
				cmnLib.enterDataInTextBox(managePerson.PaternalLastNameLocal, exl.read(strDataSheetName, dataRow, "LocalParentalLastName"), true);
				rpt.generateReport("", "Correct Name", "", "", "Must Correct name", "Entered all the details", "Passed", "", true);
				TimeUnit.SECONDS.sleep(3);
				cmnLib.clickOnWebElement(managePerson.CorrectNameOKButton);
				TimeUnit.SECONDS.sleep(3);
				cmnLib.clickOnWebElement(managePerson.SubmitButton);
				
				if(cmnLib.waitForElementToBeVisible(managePerson.Warning_Popup, 20))
				{
					rpt.generateReport("", "Correct Name", "", "", "Warning message must Pop up", "Warning message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.Yes_Warning);
				}
				
				if(cmnLib.waitForElementToBeVisible(managePerson.Confirmation_PopUp, 20))
				{
					rpt.generateReport("", "Correct Name", "", "", "Confirmation message must Pop up", "Confirmation message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.Ok_Confirmation);
				}else {
					rpt.generateReport("", "Correct Name", "", "", "Confirmation message must Pop up", "Confirmation message has not Popped up", "Failed", "", true);
