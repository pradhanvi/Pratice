...
/**
Script Name			: HCM_CHILE_T003_2_View_Update_PrsnlInfo_DeleteRecord
Script Description	: View and Update Personal Information - Delete record
Track/Module		: HCM
Sub Track			: 
...
public class HCM_CHILE_T003_2_View_Update_PrsnlInfo_DeleteRecord extends TestBase {
	...
	@Test
	public void HCM_CHILE_T003_2_View_Update_PrsnlInfo_DeleteRecord_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T003_2_View_Update_PrsnlInfo", "Delete record");

		String strDataSheetName = "T003_2_ViewUpdate_PrsnInfo";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T003_2_View_Update_PrsnlInfo_DeleteRecord", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T003_2_View_Update_PrsnlInfo_DeleteRecord", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Verify the Entered Person Search Record Exists
			if(Person_Management.verifySearchedRecordExists(strPersonNumber) == false) {
				...
				Assert.fail("Person Number in Search Person page could not be entered");
			}else
			{
				//Pass
		...
		//Selecting Options from Edit dropdown
				...
				managePerson.clickNameEditSelectDelete();
				TimeUnit.SECONDS.sleep(3);
				
		//Update Name
				if(cmnLib.waitForElementToBeVisible(managePerson.DeleteDialogYesButton, 10))
				{
					rpt.generateReport("", "Delete record", "", "", "Delete message must Pop up", "Warning message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.DeleteDialogYesButton);
				}else {
					rpt.generateReport("", "Delete record", "", "", "Delete message must Pop up", "Confirmation message has not Popped up", "Failed", "", true);
					Assert.fail("Error during deletion");
				}
				...
				{
					rpt.generateReport("", "Delete record", "", "", "Warning message must Pop up", "Warning message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.DeleteSubmitYesButton);
				}
				
				if(cmnLib.waitForElementToBeVisible(managePerson.Confirmation_PopUp, 10))
				{
					rpt.generateReport("", "Delete record", "", "", "Confirmation message must Pop up", "Confirmation message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.Ok_Confirmation);
				}else {
					rpt.generateReport("", "Delete record", "", "", "Confirmation message must Pop up", "Confirmation message has not Popped up", "Failed", "", true);
