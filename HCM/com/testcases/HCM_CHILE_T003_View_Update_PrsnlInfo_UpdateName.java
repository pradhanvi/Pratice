...
/**
Script Name			: HCM_CHILE_T003_View_Update_PrsnlInfo_UpdateName
Script Description	: View and Update Personal Information - Update Name
Track/Module		: HCM
Sub Track			: 
...
public class HCM_CHILE_T003_View_Update_PrsnlInfo_UpdateName extends TestBase{


	String strModule = "HCM";
	
	@Test
	public void HCM_CHILE_T003_View_Update_PrsnlInfo_UpdateName_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T003_View_Update_PrsnlInfo", "Update Name");

		String strDataSheetName = "T003_ViewUpdate_PrsnInfo";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T003_View_Update_PrsnlInfo_UpdateName", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T003_View_Update_PrsnlInfo_UpdateName", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Selecting Options from Edit dropdown
				...
				managePerson.clickNameEditSelectUpdate();
		...
		//Update Name
				...
				rpt.generateReport("", "Update Name", "", "", "Must update name", "Entered all the details", "Passed", "", true);
				TimeUnit.SECONDS.sleep(3);
				cmnLib.clickOnWebElement(managePerson.UpdateNameOKButton);
				TimeUnit.SECONDS.sleep(2);
				cmnLib.clickOnWebElement(managePerson.SubmitButton);
				
				if(cmnLib.waitForElementToBeVisible(managePerson.Warning_Popup, 10))
				{
					rpt.generateReport("", "Update Name", "", "", "Warning message must Pop up", "Warning message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
				...
				{
					rpt.generateReport("", "Update Name", "", "", "Confirmation message must Pop up", "Confirmation message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.Ok_Confirmation);
				}else {
					rpt.generateReport("", "Update Name", "", "", "Confirmation message must Pop up", "Confirmation message has not Popped up", "Failed", "", true);
