...
/**
Script Name			: HCM_CHILE_T004_4_ManageCommunicationMethods_DeletePhoneFrmEmpDir
Script Description	: Manage Communication Methods - Delete Phone frm Emp Dir
Track/Module		: OTM
Sub Track			: 
...
public class HCM_CHILE_T004_4_ManageCommunicationMethods_DeletePhoneFrmEmpDir extends TestBase {
	String strModule = "HCM_EMP";
	
	@Test
	public void HCM_CHILE_T004_4_ManageCommunicationMethods_DeletePhoneFrmEmpDir_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T004_4_ManageCommunicationMethods", "Delete Phone frm Emp Dir");

		String strDataSheetName = "T004_4_ManageCommMthds";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T004_4_ManageCommunicationMethods_DeletePhoneFrmEmpDir", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T004_4_ManageCommunicationMethods_DeletePhoneFrmEmpDir", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Click on Delete Phone number
			cmnLib.clickOnWebElement(managecomm.EditPhoneNum);
			
		//Delete Phone number
			cmnLib.clickOnWebElement(managecomm.Delete_PhoneNum);
			
			
			rpt.generateReport("", "Delete Phone number", "", "", "Should delete aphone number", "Deleted phone number", "Passed", "", true);
			TimeUnit.SECONDS.sleep(3);
			cmnLib.clickOnWebElement(managecomm.OKButton_AddPhone);
			if(!cmnLib.waitForElementToBeVisible(managecomm.ErrorPopUp, 15))
			{
				rpt.generateReport("", "Delete Phone number", "", "", "Successfuly deleted phone number", "Successfuly deleted phone number", "Passed", "", true);
				TimeUnit.SECONDS.sleep(2);
			}else {
				rpt.generateReport("", "Delete Phone number", "", "", "Successfuly deleted phone number", "Error occured during adding a phone number", "Failed", "", true);
				TimeUnit.SECONDS.sleep(2);
				Assert.fail("Error occured during deleted a phone number");
			}
}
}
}
