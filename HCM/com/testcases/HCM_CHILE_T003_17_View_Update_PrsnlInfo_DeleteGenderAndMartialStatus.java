...
/**
Script Name			: HCM_CHILE_T003_17_View_Update_PrsnlInfo_DeleteGenderAndMartialStatus
Script Description	: View and Update Personal Information - Delete Gender and Martial status
Track/Module		: OTM
Sub Track			: 
...
public class HCM_CHILE_T003_17_View_Update_PrsnlInfo_DeleteGenderAndMartialStatus extends TestBase{


	String strModule = "HCM";
	
	@Test
	public void HCM_CHILE_T003_17_View_Update_PrsnlInfo_DeleteGenderAndMartialStatus_TC() throws Throwable {
		rpt = new ReportGeneration("HCM_CHILE_T003_17_View_Update_PrsnlInfo", "Delete Gender and Martial status");

		String strDataSheetName = "T003_17_ViewUpdate_PrsnInfo";
		...
		if(cmnLib.waitForElementToBeVisible(homePage_HCM.HomePage_HomeIcon) == true)
		{
			...
			//PASS
			rpt.generateReport("HCM_CHILE_T003_17_View_Update_PrsnlInfo_DeleteGenderAndMartialStatus", "Enter URL", "", "Password: "+hashmap.get("URL_"+strModule), "URL should be entered", "URL is entered", "Info", "", true );
		...
		{
			//FAIL
			rpt.generateReport("HCM_CHILE_T003_17_View_Update_PrsnlInfo_DeleteGenderAndMartialStatus", "Login to Appication", "", "UserName: "+exl.read("LoginDetails", 1, "UserName"), "Login Should be successful", "Login Is Not Successful", "Failed", "", true );
			Assert.fail("Login Un-Successful");
		}
		...
		//Delete Gender and Martial status
				
				managePerson.clickGenderMartialStatusEditSelectDelete();
				
				
				cmnLib.waitForElementToBeVisible(managePerson.DeleteConfirmationPOPUp, 30);
				cmnLib.clickOnWebElement(managePerson.DeleteConfirmationPOPUp);
				rpt.generateReport("", "Delete Gender and Martial status", "", "", "Delete Confirmation Popup window should appear", "Delete Confirmation Popup window appeared", "Passed", "", true);
				TimeUnit.SECONDS.sleep(3);
				cmnLib.clickOnWebElement(managePerson.DeleteConfirmationPOPUpOKButton);
				
				
		//Error message Popup
				if(cmnLib.waitForElementToBeVisible(managePerson.ErrorPopUpDelete, 50))
				{
					rpt.generateReport("", "Delete Gender and Martial status", "", "", "Error message must Pop up", "Error message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.ErrorPopUpDeleteOKButton);
				}
				
					
		//Submit 
				cmnLib.waitForElementToBeVisible(managePerson.SubmitButton, 60);
				cmnLib.clickOnWebElement(managePerson.SubmitButton);
				
				if(cmnLib.waitForElementToBeVisible(managePerson.Warning_Popup, 30))
				{
					rpt.generateReport("", "Delete Gender and Martial status", "", "", "Warning message must Pop up", "Warning message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
		...
		//Confirmation
				...
				{
					rpt.generateReport("", "Delete Gender and Martial status", "", "", "Confirmation message must Pop up", "Confirmation message is Popping up", "Passed", "", true);
					TimeUnit.SECONDS.sleep(3);
					cmnLib.clickOnWebElement(managePerson.Ok_Confirmation);
				}else {
					rpt.generateReport("", "Delete Gender and Martial status", "", "", "Confirmation message must Pop up", "Confirmation message has not Popped up", "Failed", "", true);
